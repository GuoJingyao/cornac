...
rawdata = cornac.datasets.movielens.load_1m(reader=Reader(bin_threshold=1.0))
...
initialBi = numpy.zeros(len(items))
...
pmf_pretrain = BPR_seperable(k=10, max_iter=maxiter, learning_rate=0.001, lambda_reg=0.001,
                             init_params={'V': numpy.copy(initialV), 'U': numpy.copy(initialU), 'Bi': numpy.copy(initialBi),
                                          'globalD_users': globalD_users, 'globalD_items': globalD_items})
# pretrain V
...
trainedBi = numpy.copy(initialBi)
for oldindex, newindex in Strain_Ttest.train_set.iid_map.items():
    trainedBi[globalD_items.get(oldindex)] = pmf_pretrain.i_biases[newindex]
...
pmf_baseline = BPR_seperable(k=10, max_iter=maxiter, learning_rate=0.001, lambda_reg=0.001,
                             init_params={'V': numpy.copy(initialV), 'U': numpy.copy(initialU), 'Bi': numpy.copy(initialBi),
                                          'globalD_users': globalD_users, 'globalD_items': globalD_items}, verbose=False)
pmf_fixV = BPR_seperable(k=10, max_iter=maxiter, learning_rate=0.001, lambda_reg=0.001, fixedParameter='V',
                         init_params={'V': numpy.copy(trainedV), 'U': numpy.copy(initialU), 'Bi': numpy.copy(initialBi),
                                      'globalD_users': globalD_users, 'globalD_items': globalD_items}, verbose=False)
