...
def pmf_seperable(X, n_X, d_X, k, n_epochs=100, lamda=0.001, learning_rate=0.001, gamma=0.9, init_params=None):
    ...
    for epoch in range(n_epochs):
        for u_, i_, val in X:
            ...
            V[i_, :] += learning_rate * (
                    grad_v[i_, :] / (np.sqrt(cache_v[i_, :]) + eps))  # Update item factor
